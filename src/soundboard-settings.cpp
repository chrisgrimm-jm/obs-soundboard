...
void SoundboardSettings::buildUI()
{
    ...
    auto *sceneHint = new QLabel(
        "Choose the OBS scene that holds your sound clips. Add each clip in "
    ...
    auto *setupHint = new QLabel(
        "Click below to automatically nest your Soundboard scene at the top "
...
void SoundboardSettings::populateSceneCombo()
{
    ...
    if (idx < 0) idx = m_sceneCombo->count() ? 0 : -1;
    if (idx >= 0) m_sceneCombo->setCurrentIndex(idx);
    else m_sceneCombo->addItem(current);
}
...
{
    SoundboardManager::instance().setSceneName(
