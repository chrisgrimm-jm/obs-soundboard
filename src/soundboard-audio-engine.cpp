...
SoundboardAudioEngine::EngineHandle *SoundboardAudioEngine::engineFor(const std::string &deviceName)
{
	...
	if (haveId)
		config.pPlaybackDeviceID = &targetId;
...
void SoundboardAudioEngine::play(const std::string &clipKey, const std::vector<std::string> &deviceNames,
				 const std::string &filePath, double startSec, double durationSec)
{
	if (deviceNames.empty() || filePath.empty())
		return;
	...
	for (const auto &deviceName : deviceNames) {
		...
		ma_result r = ma_sound_init_from_file(&handle->engine, filePath.c_str(), MA_SOUND_FLAG_DECODE, nullptr,
						      nullptr, &active->sound);
		if (r != MA_SUCCESS) {
			blog(LOG_WARNING, "[soundboard] Failed to load '%s' for device '%s'", filePath.c_str(),
			     deviceName.c_str());
			delete active;
			continue;
		}
